        ...
        <div class="table-responsive my-4" v-if="articles.length!==0">
          <table class="table table-hover table-sm table-light" id="tbl_articles" >
            ...
            <tr v-for="(article,index) in articles.data" :key="article.id">
              ...
              <td >{{ ++index }}</td>
              <td>{{ article.title }}</td>
              <td class="articleDesc">{{ article.desc }}</td>
              <td >
          ...
          <ul v-if="articles.data" class="pagination">
            <li v-if="articles.current_page !== 1" :class="{'disabled':articles.current_page === 1}" class="page-item first-item">
              <router-link :to="{ query: { page: 1 }}" class="page-link">اولین صفحه</router-link>
            </li>
            <li v-if="articles.current_page !== 1" :class="{'disabled':articles.current_page === 1}" class="page-item previous-item">
              <router-link :to="{ query: { page: articles.current_page - 1 }}" class="page-link">قبلی</router-link>
            </li>
            <li v-for="page in pages" :key="page" :class="{'active':articles.current_page === page}" class="page-item number-item">
              <router-link :to="{ query: { page: page }}" class="page-link">{{page}}</router-link>
            </li>
            <li v-if="articles.current_page !== articles.last_page" :class="{'disabled':articles.current_page === articles.last_page}" class="page-item next-item">
              <router-link :to="{ query: { page: articles.current_page + 1 }}" class="page-link">بعدی</router-link>
            </li>
            <li v-if="articles.current_page !== articles.last_page" :class="{'disabled':articles.current_page === articles.last_page}" class="page-item last-item">
...
<script>
  ...
  export default {
    ...
    methods:{
      setPages(){
        let numberOfPages = Math.ceil((this.articles).last_page / (this.articles).per_page);
        this.pages = [];
